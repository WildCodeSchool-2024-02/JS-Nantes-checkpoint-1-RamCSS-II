...
function findAdults(persons) {
  const maleAdults = persons.filter((person) => { return (person.age>=18 && person.sex === 'male')});
  const femaleAdults = persons.filter((person) => { return (person.age>=18 && person.sex === 'female')});
